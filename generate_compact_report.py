"""
generate_compact_report.py
Builds an exact 18-page academic project report in Word (.docx) and exports to PDF.
...
def style_table(tbl, col_widths, headers, data, header_bg="1A365D", font_size=8.5):
    tbl.alignment = WD_TABLE_ALIGNMENT.CENTER
...
def add_p(doc, text, bold_prefix=None, style='Normal'):
...
def create_report():
    ...
    # 0.75 inch margins gives 7.0 x 9.5 inches printable area
    ...
    p_deg.paragraph_format.space_after = Pt(40)
    ...
    p_sig = add_p(doc, "Harshal | Department of Computer Science & Engineering | Academic Year: 2026", bold_prefix="Candidate: ")
    ...
    add_p(doc, "The structural layout of this academic project report is organized as follows:")
    ...
    style_table(t_idx, [1.0, 4.8, 1.2], idx_headers, idx_data, font_size=9)
    doc.add_page_break()
    ...
    add_p(
        doc,
        "Inventory refers to the physical goods, raw materials, and finished stock maintained by a business to support customer "
        "demand and commercial operations. Because inventory directly ties up working capital, managing stock balances is one of "
        "the most crucial responsibilities in retail, wholesale, and production businesses."
    )
    add_heading_2(doc, "1.1 The Need for a Computerized Inventory System")
    add_p(
        doc,
        "In manual bookkeeping, clerks record stock in paper notebooks. When customer inquiries occur, clerks must physically check shelves "
        "or search through ledger books. Miscalculations often cause unexpected stockouts, resulting in lost revenue and dissatisfied customers. "
        "Conversely, over-purchasing slow-moving items ties up liquidity and increases storage costs."
    )
    ...
    for b_prefix, b_text in objs:
        add_p(doc, b_text, bold_prefix=b_prefix, style='List Bullet')
    ...
    style_table(t_scp, [1.4, 2.8, 2.8], scp_headers, scp_data, font_size=8.5)
    doc.add_page_break()
    ...
    style_table(t_hw, [1.8, 2.6, 2.6], hw_headers, hw_data, font_size=8.5)
    ...
    style_table(t_sw, [1.8, 2.4, 2.8], sw_headers, sw_data, font_size=8.5)
    doc.add_page_break()
    ...
    for b_pre, b_text in flaws:
        add_p(doc, b_text, bold_prefix=b_pre, style='List Bullet')
    ...
    style_table(t_cmp, [1.8, 2.6, 2.6], cmp_headers, cmp_data, font_size=8.5)
    doc.add_page_break()
    ...
    ff_items = [
        ("1. On-Site Observation:",
         ...
         "highlighted that receiving multi-item shipments was the single largest bottleneck."),
    ...
    for b_pre, b_text in ff_items:
        add_p(doc, b_text, bold_prefix=b_pre)
    ...
    style_table(t_tier, [1.8, 2.4, 2.8], tier_headers, tier_data, font_size=8.5)
    ...
    style_table(t_erd, [1.2, 0.9, 2.5, 2.4], erd_headers, erd_data, font_size=8.5)
    ...
    style_table(t_dfd1, [0.8, 1.8, 1.4, 1.5, 1.5], dfd1_headers, dfd1_data, font_size=8.0)

    add_heading_2(doc, "10.3 DFD Level 2 (Purchase Bill & Stock Increment Workflow)")
    add_p(doc, "1. Staff enters supplier ID, invoice number, and date -> System validates header and flushes invoice record.")
    add_p(doc, "2. For each product row, system validates quantity and cost -> Inserts PurchaseItem row.")
    add_p(doc, "3. System executes atomic update: Product.quantity += item.quantity and updates Product.purchase_price.")
    add_p(doc, "4. Transaction commits atomically. Any unexpected exception triggers a full rollback, preserving stock integrity.")
    doc.add_page_break()

    # -------------------------------------------------------------
    # PAGE 14: 11. DATA DICTIONARY
    # -------------------------------------------------------------
    add_heading_1(doc, "11. Data Dictionary")
    add_p(
        doc,
        "The Data Dictionary defines field-level metadata for all 7 tables in the SQLite database, "
        "detailing field names, data types, sizes, integrity constraints, and operational descriptions."
    )
    
    dict_headers = ["Table Name", "Field Name", "Data Type", "Constraints", "Description"]
    dict_data = [
        ["USERS", "id", "Integer (4B)", "PK, Auto-Increment", "Unique user account ID"],
        ["USERS", "username", "Varchar(64)", "Unique, Not Null, Indexed", "Login username"],
        ["USERS", "password_hash", "Varchar(255)", "Not Null", "Salted PBKDF2/scrypt password hash"],
        ["USERS", "role", "Varchar(20)", "Default 'admin'", "Privilege level ('admin' or 'staff')"],
        ["CATEGORIES", "id / name", "Int / Varchar", "PK / Unique, Not Null", "Category ID and unique category name"],
        ["SUPPLIERS", "id / name", "Int / Varchar", "PK / Not Null", "Vendor ID and contact person name"],
        ["SUPPLIERS", "company / gst", "Varchar(150/30)", "Nullable", "Business name and legal GST tax number"],
        ["PRODUCTS", "id", "Integer (4B)", "PK, Auto-Increment", "Unique product record ID"],
        ["PRODUCTS", "product_code", "Varchar(20)", "Unique, Not Null, Indexed", "Auto-generated SKU (e.g. PRD1001)"],
        ["PRODUCTS", "name / brand", "Varchar(150/100)", "Not Null / Nullable", "Product name and brand/manufacturer"],
        ["PRODUCTS", "category_id", "Integer (4B)", "FK (categories.id)", "Reference to parent category"],
        ["PRODUCTS", "purchase_price", "Float (8B)", "Not Null, Default 0.0", "Unit purchase cost paid to vendor"],
        ["PRODUCTS", "selling_price", "Float (8B)", "Not Null, Default 0.0", "Retail price charged to customers"],
        ["PRODUCTS", "quantity", "Integer (4B)", "Not Null, Default 0", "Current on-hand physical stock count"],
        ["PURCHASES", "id / invoice_no", "Int / Varchar(50)", "PK / Unique, Not Null", "Invoice ID and vendor invoice number"],
        ["PURCHASES", "supplier_id", "Integer (4B)", "FK (suppliers.id)", "Reference to supplying vendor"],
        ["PURCHASES", "total_amount", "Float (8B)", "Not Null, Default 0.0", "Total financial invoice value"],
        ["PURCHASE_ITEMS", "purchase_id / prod_id", "Int (4B)", "FK (purchases.id / products.id)", "References to parent invoice and item"],
        ["PURCHASE_ITEMS", "quantity / cost", "Int / Float", "Not Null", "Quantity received and unit cost price"],
        ["STOCK_ADJUST", "product_id / change", "Int (4B)", "FK (products.id) / Signed Int", "Product reference and delta count (+/-)"],
        ["STOCK_ADJUST", "reason / adjusted_by", "Varchar(255/64)", "Nullable", "Reason for adjustment and staff username"]
    ]
    t_dict = doc.add_table(rows=1, cols=5)
    style_table(t_dict, [1.3, 1.3, 1.2, 1.7, 1.5], dict_headers, dict_data, font_size=7.5)
    doc.add_page_break()
    ...
    add_p(doc, "1. Automated Sequential SKU Assignment: Reads highest existing 'PRD####' code and increments numerical suffix (PRD1001 -> PRD1002).")
    add_p(doc, "2. Atomic Purchase Stock Increment: Creates invoice lines and increments Product.quantity within a single atomic database transaction.")
    add_p(doc, "3. Negative Stock Prevention: Verifies requested decrement does not exceed on-hand balance before executing a manual reduction.")
    ...
    style_table(t_tst, [0.6, 0.9, 1.4, 1.1, 1.2, 1.2, 0.6], test_headers, test_data, font_size=7.5)
    doc.add_page_break()
